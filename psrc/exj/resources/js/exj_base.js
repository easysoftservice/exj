/*
 Info:   Base para Sistema App. Usando framework Ext 3.3.1
...
Exj.replaceVarsGenerals = function(html) {
    if (!html || !html.replace) {
        return html;
    }

    html = html.replace('{navigator.userAgent}', navigator.userAgent);
    html = html.replace('{navigator.platform}', navigator.platform);

    html = html.replace('{Exj.calcWidth()}', Exj.calcWidth());
    html = html.replace('{Exj.calcHeight()}', Exj.calcHeight());
...
Exj.loadDataApp = function (config) {
    ...
    Exj.submit({
        ...
        fnSuccess: function (response) {
            ...
            // copy
            for(propDG in dataGlobal) {
                if (propDG == 'dataMenusMain') {
                    continue;
                }

                valDG = dataGlobal[propDG];

                if (propDG == 'infoFile') {
                    Exj.files.charsMaxNameFile = valDG.charsMaxNameFile;
                    Exj.files.maxSizeUpload = valDG.maxSizeUpload;
                }
                else if(propDG == 'pgValuesMap') {
                    Exj.pg.valuesMap = valDG;
                }
                else if(propDG == 'Const') {
                    Exj.Const = valDG; // constantes de la app
                }
                else if(propDG == 'segTimeoutRep') {
                    if (valDG) {
                        Exj.Global.timeoutRep = Ext.num(valDG, 0);
                    ...
                    // console.log('propDG: ', propDG, valDG);
                    Exj.Global[propDG] = valDG;
                }
            }
