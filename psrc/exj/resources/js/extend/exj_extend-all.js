...
Ext.apply(Ext.form.VTypes, {
    ...
    daterange : function(val, field) {
        ...
        if (field.startDateField) {
            ...
            if(!start){
            	Exj.moe('No se encuentra: '+field.startDateField, ' Validación de fechas');
            	return false;
            }
            ...
            if(!end){
            	Exj.moe('No se encuentra: '+field.endDateField, ' Validación de fechas');
            	return false;
            }
    ...
    numdoc: function(val, field) {
    	...
    	if(valueNum.length <= 3){
    		fieldNum.numdocText = 'Número de documento inválido';
    		return false;
    	}
	...
	//	fieldType.clearInvalid();
		...
		if(isCedula || isRUC){
			...
			if(charInvalid !== null){
				...
				return _setMsgInvalid('No válido el caracter: '+charInvalid);
			}
		...
		*/
		...
		if (pub){
			if (digitoVerificador != d9){
				return _setMsgInvalid('RUC de la empresa del sector público es incorrecto.');
			}
			...
			*/
			...
			if(isRUC && val.length != 13){
				return _setMsgInvalid('RUC de empresa del sector público debe tener 13 dígitos y terminar en 0001, 0002, etc');
			}
		}
		else if(pri){
	...
	_isValidChars: function(val, namProp, patron, showCharInvalid){
    	...
    	if(charInvalid !== null){
    		...
    		if(charInvalid == ' '){
    			this[namProp] = 'No válido espacios en blanco';
    		}
    		else{
    			this[namProp] = 'No válido el caracter: '+charInvalid;
    		}
    ...
    /* validación de texto para nombre de archivo */
    ...
    textnamefileText: 'Inválido nombre del archivo',
	

    /* validación de texto para nombres */
    textname: function(val, field) {
    	if(val && !val.trim()){
    		if(field.fieldLabel){
    			this.textnameText = field.fieldLabel+' inválido';
    		}
    		else{
    	...
    	return this._isValidChars(val, 'textnameText', '[^A-Za-z áéíóúüñÑÁÉÍÓÚ.&(),-]');
    },
    textnameText: 'Solamente letras, espacios y .',
    
    /* validación de texto para nombres con car extras */
    textnameext: function(val, field) {
    	...
    	return this._isValidChars(val, 'textnameextText', '[^0-9A-Za-z áéíóúüñÑÁÉÍÓÚ.-/&()\'-_]');
    },
    textnameextText: 'Solamente letras, números, espacios y .-/&()-',
    

    /* validación de texto general */
    textgeneral: function(val, field) {
    	return this._isValidChars(val, 'textgeneralText', '[^0-9A-Za-z \,áéíóúüñÑÁÉÍÓÚ+)(\'\*\-\_\&\#\$\"]');
    },
    textgeneralText: 'Texto inválido',
    
    /* validación de texto para código */
    textcode: function(val, field) {
    	return this._isValidChars(val, 'textcodeText', '[^0-9A-Za-z .,/&-ñÑ]');
    },
    textcodeText: 'Código inválido',

    /* validación de texto para código */
    textcodeletters: function(val, field) {
    	return this._isValidChars(val, 'textcodelettersText', '[^A-Za-z]', false);
    },
    textcodelettersText: 'Código inválido, solo se permiten letras',
    
    /* validación de texto para código de números */
    ...
    textcodenumsText: 'Código inválido, Solo números son permitidos y .',
    
    
    /* validación de texto para campos text o memo */
    textmemo: function(val, field) {
    	return this._isValidChars(val, 'textmemoText', '[^0-9A-Za-z- .:*/áéíóúüñÑÁÉÍÓÚ,\r\n\'()@&´#_$+%!"=<>`]');
    },
    textmemoText: 'Texto inválido',
    
    /* validación de texto para teléfono */
    textfono: function(val, field) {
    	if(val && val.match('[0-9]') === null){
    		this.textfonoText = 'No válido el número de teléfono';
    		return false;
    	}
    	if(!val){
    		this.textfonoText = 'Solo números son aceptados espacios y el caracter -';
    	}
    	return this._isValidChars(val, 'textfonoText', '[^0-9 -/]', false);
    },
    textfonoText: 'Solo se aceptan números, espacios y los caracteres - /',
    
    /* validación de texto para nombres, acepta espacio en blanco */
    textzipcode: function(val, field) {
    	if(val && !val.trim()){
    		if(field.fieldLabel){
    			this.textzipcodeText = field.fieldLabel+' inválido';
    		}
    		else{
    	...
    	return this._isValidChars(val, 'textzipcodeText', '[^0-9 A-Za-záéíóúüñÑÁÉÍÓÚ.-]');
    },
    textzipcodeText: 'Valid characters letters or numbers and -',
    
    /* validación de texto para URL */
    ...
    texturlText: 'La URL no es válido',
