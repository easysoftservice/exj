...
/**
 * Empaqueta el código PHP y lo ofusca
 * Autor: Byron Córdova
 */
class PHPPacker {
	// constants
	...
	private $_ofsWords = array();
	private $_onlyOfuscar = false;
	private $_numWordsOfuscadas = 0;
	...
	public function __construct($_script, $_encoding = 62, $_fastDecode = true, $_specialChars = false, $onlyOfuscar=false)
	{
		$this->_onlyOfuscar = $onlyOfuscar;
	...
	public function pack() {
		...
		// go!
		$this->_script = $this->_pack($this->_script);
		
		return $this->_ofuscar();
	}

	private $_wordsOfuscadas=null;

	public function getWordsOfuscadas(){
		if (!$this->_wordsOfuscadas) {
			return array();
		}

		return $this->_wordsOfuscadas;
	}

	public function findWordOfuscada($word){
		$val = null;
		$wofs = $this->getWordsOfuscadas();
		if ($wofs && isset($wofs[$word])) {
			$val = $wofs[$word];
		}
...
//		$this->addOfuscarVar('$i');
		...
		$numRemplaceTotal =0;
		$this->_wordsOfuscadas = array();
		foreach ($ofsWords as $varx => $varEncode) {
			$numRemplace = 0;
			$script = str_replace($varx, $varEncode, $script, $numRemplace);
			
			if ($numRemplace) {
				$numRemplaceTotal += $numRemplace;
				$this->_wordsOfuscadas[$varx] = $varEncode;
			}
		}
		
		$this->_numWordsOfuscadas = $numRemplaceTotal;
		
		return $script;
	}
	
	public function getNumWordsOfuscadas(){
		return $this->_numWordsOfuscadas;
	}
	
	public function addOfuscarVar($name){
		$name = trim($name);
		if (!$name) {
			return false;
		}
		
		if (isset($this->_ofsWords[$name])) {
			return true;
		}
		
		
		$nChars = strlen($name);
		$varEncode = '$_'.substr($name, $nChars-1, 1);
		$varEncode .= $nChars.count($this->_ofsWords);
		
		$this->_ofsWords[$name] = $varEncode;
		
		return true;
	}
	
	
	public function addOfuscarClass($name){
		$name = trim($name);
		if (!$name) {
			return false;
		}
		...
		$varEncode = '_'. ord(substr($name, $nChars-1, 1));
		$varEncode .= $nChars.count($this->_ofsWords);
	...
	private function _basicCompression($script) {
		...
		// NOTE: para php no se admite esto
		// $parser->add('/;+\\s*([};])/', '$2');
	...
	private function _bootStrap($packed, $keywords) {
		...
		$unpackPacker = new PHPPacker($unpack, 0, false, true);
...
/**
 * clase: ParsePHPMaster
 *
 */
class ParsePHPMaster {
